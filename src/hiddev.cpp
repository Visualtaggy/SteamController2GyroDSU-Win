    ...
    : dsuSlot(dsuSlot), pid(pid), serial(std::move(serial)), dev_(dev)
{
    ...
    lastLizard_ = now - 10s; // Force immediate refresh
    lastImu_    = now - 10s;
...
bool HidSlot::readOne(ControllerState& out) {
    ...
    // Payload starts after report ID byte
    const uint8_t* p = buf + 1;
    int plen = n - 1;

    // Need at least IMU_OFFSET + 16 bytes
    if (plen < (int)(IMU_OFFSET + 16)) return false;

    // Parse buttons / sticks / triggers
    ...
    // Scale: ±2g range → divide by 16384; ±2000 dps range → divide by 16.384
    float rawA[3] = {
    ...
    while (cur) {
        if (isTritonPid(cur->product_id) && cur->usage_page >= 0xFF00) {
            ...
            if (!serial.empty() && !isActive(active, serial)) {
                ...
                if (slot < 0) {
                    fprintf(stderr, "scanner: all 4 slots full, ignoring %s\n",
                            serial.c_str());
                } else {
                    hid_device* dev = hid_open_path(cur->path);
                    if (dev) {
                        fprintf(stderr,
                            "scanner: opened PID %04X (%s) serial=%s → slot %d\n",
                        ...
                        fprintf(stderr, "scanner: hid_open_path failed for %s\n",
                                cur->path);
                    }
                }
            }
...
void HidManager::probe() {
    ...
    while (cur) {
        ...
        fprintf(stderr,
            "  PID %04X usage_page=0x%04X path=%s%s\n",
