...
void HidSlot::sendFeatureReport(uint8_t setting, uint16_t value) {
    // +1 spare byte: the Windows HID stack (HidD_SetFeature) requires the
    // buffer to match the device's declared feature report length, which can
    // be FEATURE_REPORT_SIZE or FEATURE_REPORT_SIZE+1 depending on transport.
    uint8_t buf[FEATURE_REPORT_SIZE + 1] = {};
    ...
    buf[4] = value & 0xFF;
    buf[5] = (value >> 8) & 0xFF;
    if (hid_send_feature_report(dev_, buf, FEATURE_REPORT_SIZE) < 0)
        hid_send_feature_report(dev_, buf, FEATURE_REPORT_SIZE + 1);
}
